...
public sealed record SchemaMigration(string Id, string Description, string Sql);

public static class AetherXivMigrations
{
    public const string InitialSchemaId = "20260701_000001_aetherxiv2_initial_schema";

    public static SchemaMigration InitialSchema { get; } = new(
        InitialSchemaId,
        "Initial AetherXIV 2.0 auth/session/character/world/actor/provenance schema.",
        InitialSchemaSql);

    public static IReadOnlyList<SchemaMigration> All { get; } = new[] { InitialSchema };
...
CREATE TABLE IF NOT EXISTS characters (
  ...
  CONSTRAINT fk_characters_world FOREIGN KEY (world_id) REFERENCES worlds(world_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS provenance_refs (
  ...
  KEY idx_provenance_status (evidence_status),
  KEY idx_provenance_source (source_type, source_ref)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS static_actor_spawns (
  ...
  KEY idx_static_actor_spawns_zone (zone_id),
  CONSTRAINT fk_static_actor_spawns_zone FOREIGN KEY (zone_id) REFERENCES zones(zone_id),
  CONSTRAINT fk_static_actor_spawns_provenance FOREIGN KEY (provenance_id) REFERENCES provenance_refs(provenance_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS battle_npc_groups (
  ...
  KEY idx_battle_npc_groups_zone (zone_id),
  CONSTRAINT fk_battle_npc_groups_pool FOREIGN KEY (pool_id) REFERENCES battle_npc_pools(pool_id),
  CONSTRAINT fk_battle_npc_groups_zone FOREIGN KEY (zone_id) REFERENCES zones(zone_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS battle_npc_spawns (
  ...
  KEY idx_battle_npc_spawns_group (group_id),
  CONSTRAINT fk_battle_npc_spawns_group FOREIGN KEY (group_id) REFERENCES battle_npc_groups(group_id),
  CONSTRAINT fk_battle_npc_spawns_provenance FOREIGN KEY (provenance_id) REFERENCES provenance_refs(provenance_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS script_modules (
  ...
  CONSTRAINT fk_script_modules_provenance FOREIGN KEY (provenance_id) REFERENCES provenance_refs(provenance_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
