...
For local Ubuntu/Debian build machines, `tools/bootstrap-ubuntu-build.sh --yes` installs basic distro Wine and Winetricks packages when Wine is missing, initializes Echo Gate's default prefix, sets Windows 7 mode, and installs `d3dx9_41`. If `wine` is already present on `PATH`, the helper reuses that detected Wine and does not request Wine packages from apt. Use `--wine-source winehq` to add WineHQ's Ubuntu package source and install WineHQ Stable instead of distro Wine when no Wine runtime is already detected. Use `--no-client-runtime` to install Wine packages without preparing the prefix, or `--no-wine` for server-only build machines.
