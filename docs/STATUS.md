...
| Echo Gate Runtime tab | Working | Detects and validates runtimes, supports custom runtime paths, and exposes managed runtime controls. |
...
| Managed runtime catalog | Partial | Service contract exists; hosted runtime archives and release process still need production packaging. |
